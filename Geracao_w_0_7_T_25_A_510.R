# ==============================================================================
# Geracao_w_0_7_T_25_A_510.R  —  SEM efeito espacial, w=0.7, T=25, A=510
# ==============================================================================
library(sf)
library(geobr)
library(nimble)
library(dplyr)
library(spdep)
cat("--- Iniciando Simulação SEM Efeito Espacial ICAR (A=510) ---\n")

# Carregar 510 regiões imediatas (2017)
reg_shape <- read_immediate_region(year = 2017, simplified = TRUE)
reg_shape <- reg_shape %>% rename(code_imediata = code_immediate)
reg_shape$code_imediata <- as.numeric(reg_shape$code_imediata)

nb          <- poly2nb(reg_shape, queen = TRUE)
num_vec     <- as.integer(sapply(nb, length))
adj_vec     <- as.integer(unlist(nb))
n_adj_val   <- length(adj_vec)
weights_vec <- rep(1.0, n_adj_val)
N_regions   <- nrow(reg_shape)  # 510

stopifnot("adj/num inconsistentes" = sum(num_vec) == n_adj_val)
cat("Verificação espacial OK: N =", N_regions, "| sum(num) =", n_adj_val, "\n")
...
# Clusters aleatórios (sem dados reais para 510 regiões)
h_cumulativo <- matrix(0, nrow = N_regions, ncol = K)
cluster_ids  <- sample(1:K, N_regions, replace = TRUE)
for(i in 1:N_regions) h_cumulativo[i, 1:cluster_ids[i]] <- 1
