# ==============================================================================
# Estudo_MonteCarlo_NonSpatial_A75.R
# Extração completa de Métricas Estáticas (ESS, Bias, etc) e Resumos Dinâmicos
# Ajustado para ler a pasta dados_montecarlo_A75
# ==============================================================================
rm(list = ls())
library(nimble)
library(coda)
...
# Pasta de saída exclusiva para A=75
dir.create(file.path(dir_projeto, "resultados_MC_NonSpatial_A75"), showWarnings = FALSE)
...
worker_fun <- function(rep_indices, T_val, w_fix, niter, nburnin, nchains, thin, dir_base) {
  ...
  # AQUI MUDA: apontando para os dados recém-gerados de A=75
  pasta_dados <- file.path(dir_base, "dados_montecarlo_A75")
  
  arquivo_base <- file.path(pasta_dados, sprintf("replica_T%d_rep%02d.rds", T_val, 1))
  if(!file.exists(arquivo_base)) stop(paste("Ficheiro não existe:", arquivo_base))
  
  dados_base <- readRDS(arquivo_base)
  C <- dados_base$constants
  A_val <- C$n_regions # Lê automaticamente 75
  p <- C$p; K <- C$K
  ...
  for(rep in rep_indices) {
    ...
    # --- MÉTRICAS DINÂMICAS GLOBAIS (Lambda) ---
    ...
    lt_true_vec <- as.vector(t(dados_rep$lambda_true)) 
...
for(i in 1:nrow(scenarios)) {
  ...
  write_csv(df_estaticos, file.path(dir_projeto, "resultados_MC_NonSpatial_A75", sprintf("Estaticos_T%d_w%.1f.csv", S$T_val, S$w_fix)))
  write_csv(df_dinamicos, file.path(dir_projeto, "resultados_MC_NonSpatial_A75", sprintf("DinamicosGlobais_T%d_w%.1f.csv", S$T_val, S$w_fix)))
}

stopCluster(cl)
cat("\nSimulação concluída com sucesso para A=75!\n")