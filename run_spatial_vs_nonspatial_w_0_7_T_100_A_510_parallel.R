# ==============================================================================
# run_spatial_vs_nonspatial_w_0_7_T_100_A_510_parallel.R
# A=510: monitoring seletivo de lambda para controle de memória
# ==============================================================================
rm(list = ls())
...
# ------ 1. Dados ------
source("Geracao_w_0_7_T_100_A_510.R")
...
# Regiões monitoradas (12 representativas de 510)
REGIONS_INTEREST <- c(1,50,100,150,200,250,300,350,400,450,500,n_regions)
# Nomes dos lambdas selecionados — construído aqui para exportar ao cluster
...
run_model <- function(model_type, output_dir) {
  ...
  # Monitoring seletivo — apenas 12 regiões × n_times lambdas (A=510 requer esta precaução)
  ...
  samples <- runMCMC(Cmcmc,niter=niter,nburnin=nburnin,nchains=nchains,thin=10,
  ...
  if(is_spatial){
    ...
    ESS_tau <- tau_m$ESS
    # Scatter s — só para os primeiros 100 para legibilidade
  ...
  n_col_lam <- length(unique(lambda_summary$Region))
...
output_dir <- "C:/Users/vlara/OneDrive/Estatistica UFMG/Mestrado/Pesquisa/Simulacao/main/resultados_spatial_vs_nonspatial_w_0_7_T_100_A_510"
