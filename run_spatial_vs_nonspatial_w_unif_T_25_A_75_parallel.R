# ==============================================================================
# run_spatial_vs_nonspatial_w_unif_T_25_A_75_parallel.R
# A=75: seleção de 12 regiões (3 por cluster) e monitoramento seletivo
# ==============================================================================
rm(list = ls())
...
# ------ 1. Dados ------
source("Geracao_w_unif_T_25_A_75.R")
...
# Determinar cluster de cada região a partir da matriz h (cumulativa)
...
# Nomes dos lambdas selecionados
...
run_model <- function(model_type, output_dir) {
  ...
  epsilon_by_cluster <- tapply(1:n_regions, cluster_ids, function(regs) {
    colMeans(epsilon_draws[, regs, drop = FALSE])  # média por região, mas é igual para todas
  })
  # Como as médias dentro do cluster são idênticas, pegamos o primeiro valor
  epsilon_cluster_mean <- sapply(epsilon_by_cluster, function(x) x[1])
  # Expandir para cada região
  epsilon_mean <- epsilon_cluster_mean[cluster_ids]
  
  # HPD também por cluster (usando todas as regiões do cluster para maior precisão)
  ...
  # Lambdas — usar LAMBDA_MONITORS (já filtrado por REGIONS_INTEREST)
  ...
  true_epsilon <- 1 - apply(constants$h * gamma_true, 1, sum)  # vetor de N
...
output_dir <- "C:/Users/vlara/OneDrive/Estatistica UFMG/Mestrado/Pesquisa/Simulacao/main/resultados_spatial_vs_nonspatial_w_unif_T_25_A_75"
...
build_compare_plot <- function(tipo, titulo_expr) {
  ...
  if (is.null(eps_list) || nrow(eps_list) == 0) {
    ...
    # Para cada região, extraímos cluster e médias de ε
